...
class NERHead(nn.Module):
    ...
    def forward(self, inputs, batch=None):
        ...
        if labels is not None:
           ...
           # NOTE: for each example in batch, it actually not return labels of length seq_len, but only return prediction for tokens
           # that have crf_mask==1, so we process the raw_predictions by taking predictions with crf_mask==1
           # (crf_mask==0 for spesicel tokens (CLS,SEP), padding tokens, and subtokens that splitted by the tokinizer (has '##' symbol before them))
...
class RelationClassificationHead(nn.Module):
    ...
    def forward(self, inputs, batch=None):
        ...
        e2_start_embedding = last_hidden_state_vectors[torch.arange(batch_size), e2_start, :] # shape: [batch_size, 768]
